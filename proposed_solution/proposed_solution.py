...
class LidarMapperApp:
    def __init__(self):
        ...
        # 每个簇：{'cx': float, 'cy': float, 'count': int, 'last_seen': float}
        self.victim_clusters: List[dict] = []
        self.confirmed: List[Tuple[float, float]] = []  # 同步自 clusters（稳定簇的中心）

        # 可调参数
        self.V_MATCH_DIST = 0.8  # m：新检测并入簇的半径阈值
        self.V_STABLE_COUNT = 3  # 次数阈值：达到即视为稳定
        self.V_STALE_TIMEOUT = 30.0  # s：多久未更新则认为该簇过期（仅清除未稳定簇）
    ...
    def _match_cluster(self, wx: float, wy: float) -> Optional[int]:
        best_idx, best_d = None, None
        for i, c in enumerate(self.victim_clusters):
            d = math.hypot(c['cx'] - wx, c['cy'] - wy)
            if d <= self.V_MATCH_DIST and (best_d is None or d < best_d):
                best_idx, best_d = i, d
        return best_idx

    def _update_cluster_running_mean(self, c: dict, wx: float, wy: float, t: float) -> None:
        n = c['count']
        c['cx'] = (c['cx'] * n + wx) / (n + 1)
        c['cy'] = (c['cy'] * n + wy) / (n + 1)
        c['count'] = n + 1
    ...
    def _rebuild_confirmed_from_clusters(self) -> None:
        new_confirmed: List[Tuple[float, float]] = []
        for c in self.victim_clusters:
            if c['count'] >= self.V_STABLE_COUNT:
                cx, cy = c['cx'], c['cy']
                # 与现有列表去重（半径阈值）
                dup = False
                for (px, py) in new_confirmed:
                    if math.hypot(px - cx, py - cy) < self.V_MATCH_DIST:
                        dup = True
                        break
                if not dup:
                    new_confirmed.append((cx, cy))
        self.confirmed = new_confirmed

    def _prune_stale_clusters(self, t: float) -> None:
        kept = []
        for c in self.victim_clusters:
            if c['count'] >= self.V_STABLE_COUNT:
                kept.append(c)  # 稳定簇保留
            else:
                if (t - c['last_seen']) <= self.V_STALE_TIMEOUT:
                    kept.append(c)
        self.victim_clusters = kept

    def add_detection(self, wx: float, wy: float, t: float) -> None:
        idx = self._match_cluster(wx, wy)
        if idx is None:
            # 新建簇（第一次观测即为中心）
            self.victim_clusters.append({
                'cx': float(wx),
                'cy': float(wy),
                'count': 1,
                'last_seen': float(t),
            ...
            # 并入最近簇（运行均值）
            self._update_cluster_running_mean(self.victim_clusters[idx], wx, wy, t)

        # 同步 confirmed & 清理陈旧未稳定簇
        self._rebuild_confirmed_from_clusters()
        self._prune_stale_clusters(t)

        # 可选：打印新近确认
        if self.confirmed:
            # 打印最后一个（通常是最新达阈值的）
            cx, cy = self.confirmed[-1]
            print(f"[{self.name}] Cluster center(s) confirmed: ({cx:.2f}, {cy:.2f})")
    ...
    def run(self):
        ...
        while self.robot.step(self.timestep) != -1:
            ...
            victim_conf = bool(found and metrics.get("conf", 0.0) >= 0.7)
            if victim_conf:
